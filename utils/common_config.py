...
def get_backbone(p):
    ...
    if p['backbone'] == 'tspL':
        if p['train_db_name'] == 'NYUD':
            from models.transformers.tsp_nyud import  vit_large_patch16_384
            backbone = vit_large_patch16_384(pretrained=True, drop_path_rate=0.15, img_size=p.TRAIN.SCALE, prompt_nums=p.prompt_nums, prompt_layers=p.prompt_layers, prompt_insertype=p.prompt_insertype) 
            ...
            from models.transformers.tsp_pascal import  vit_large_patch16_384
            backbone = vit_large_patch16_384(pretrained=True, drop_path_rate=0.15, img_size=p.TRAIN.SCALE, prompt_nums=p.prompt_nums, prompt_layers=p.prompt_layers, prompt_insertype=p.prompt_insertype) 
            backbone_channels = [1024 for _ in range(4)]
...
def get_head(p, backbone_channels, task):
    ...
    if p['head'] == 'mlp':
        from models.transformers.tsptransformer_decoder import MLPHead
...
def get_model(p):
    ...
    if p['model'] == 'TransformerNet':
        from models.transformer_tspnet import TransformerNet
